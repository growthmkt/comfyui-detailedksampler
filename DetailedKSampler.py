...
def common_ksampler(model, seed, steps, cfg, sampler_name, scheduler, positive, negative, latent, start_at_step, end_at_step, denoise=1.0):
    ...
    latent["samples"] = comfy.sample.sample(model, noise, steps, cfg, sampler_name, scheduler, positive, negative, latent["samples"], denoise=denoise, disable_noise=False, start_step=start_at_step, last_step=end_at_step, force_full_denoise=False, noise_mask=noise_mask, callback=callback, disable_pbar=disable_pbar, seed=seed)
    return latent
...
class DetailedKSampler:
    ...
    def detailed_sample(self, model, seed, steps, extra_steps, missing_steps, cfg, sampler_name, scheduler, positive, negative, latent_image, denoise, detail_level, detail_from, auto_threshold, mimic_scale, threshold_percentile, auto_freeu, b1, b2, s1, s2, copilot, second_model=None, VAE=None):
        ...
        sample_model = common_ksampler(model,seed,steps,cfg,sampler_name,scheduler,positive,negative,latent_image,0,steps,denoise)
        ...
        if detail_level != 1.0:
            if detail_from == "penultimate_step":
                noisy_latent_1 = common_ksampler(extram,seed,steps+1,cfg,sampler_name,scheduler,positive,negative,latent_image,steps,steps+1,denoise) if detail_level > 1.0 else common_ksampler(model,seed,steps,cfg,sampler_name,scheduler,positive,negative,latent_image,steps-1,steps,denoise)
            else:
                noisy_latent_1 = sample_model
            noisy_latent_2 = common_ksampler(extram,seed,total_steps,cfg,sampler_name,scheduler,positive,negative,latent_image,start_at_extra_step,total_steps,denoise)
            ...
            sample_output = common_ksampler(extram,seed,total_steps,cfg,sampler_name,scheduler,positive,negative,latent_image,start_at_extra_step,total_steps,denoise)
        model.unpatch_model()
        if second_model is not None:
            second_model.unpatch_model()
        extram.unpatch_model()
